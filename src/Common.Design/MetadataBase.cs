...
namespace System.Windows.Controls.Design.Common
{
    ...
    public class MetadataRegistrationBase
    {
        ...
        [SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes",
            Justification = "Design time dll should not fail!")]
        private static void AddTables(AttributeTableBuilder builder)
        {
            ...
            foreach (Type t in asm.GetTypes())
            {
                if (t.IsSubclassOf(typeof (AttributeTableBuilder)))
                {
                    ...
                    {
                        Debug.Assert(false,
                            string.Format(CultureInfo.InvariantCulture, "Exception in AddTables method: {0}", e));
                    }
                }
            }
        }
        ...
        private void AddDescriptions(AttributeTableBuilder builder)
        {
            ...
            foreach (XElement member in xdoc.Descendants("member"))
            {
                        ...
                        name.StartsWith("P:", StringComparison.OrdinalIgnoreCase))
                    {
                            ...
                            t.IsSubclassOf(typeof (FrameworkElement)))
                        {
                            ...
                            else
                            {
                                ...
                                if (pi != null)
                                {
                                    builder.AddCallback(t,
                                        b => b.AddCustomAttributes(propName, new DescriptionAttribute(desc)));
                                }
                            }
                        }
                    }
                ...
                {
                    Debug.Assert(false,
                        string.Format(CultureInfo.InvariantCulture, "Exception: {0}\n For member : {1}", e, member));
                }
            }
        }
