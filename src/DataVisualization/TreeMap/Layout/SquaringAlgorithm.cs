...
namespace System.Windows.Controls.DataVisualization
{
    ...
    internal class SquaringAlgorithm
    {
        ...
        private IEnumerable<Tuple<Rect, TreeMapNode>> BuildTreeMapStep()
        {
            ...
            if (horizontal)
            {
                _currentRectangle = new Rect(_currentRectangle.Left + wh, _currentRectangle.Top,
                    Math.Max(0, _currentRectangle.Width - wh), _currentRectangle.Height);
            }
            else
            {
                _currentRectangle = new Rect(_currentRectangle.Left, _currentRectangle.Top + wh, _currentRectangle.Width,
                    Math.Max(0, _currentRectangle.Height - wh));
            }
        }
