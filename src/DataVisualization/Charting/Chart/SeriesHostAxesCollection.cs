...
namespace System.Windows.Controls.DataVisualization.Charting
{
    ...
    internal class SeriesHostAxesCollection : UniqueObservableCollection<IAxis>
    {
        ...
        internal SeriesHostAxesCollection(ISeriesHost seriesHost)
        {
            ...
            PersistentAxes = new UniqueObservableCollection<IAxis>();
            ...
            : this(seriesHost)
        {
            ...
            SeriesHost = seriesHost;
            PersistentAxes = persistentAxes;
            PersistentAxes.CollectionChanged += PersistentAxesCollectionChanged;
        }
        ...
        public void PersistentAxesCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            if (e.OldItems != null)
            {
                foreach (IAxis axis in e.OldItems)
                {
                    if (Contains(axis) && !SeriesHost.IsUsedByASeries(axis))
                    {
                        Remove(axis);
                    }
                }
        ...
        protected override void RemoveItem(int index)
        {
            ...
            if (PersistentAxes.Contains(axis))
            {
                throw new InvalidOperationException(
                    Properties.Resources.SeriesHostAxesCollection_InvalidAttemptToRemovePermanentAxisFromSeriesHost);
            }
