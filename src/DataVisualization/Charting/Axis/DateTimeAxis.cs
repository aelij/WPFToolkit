...
namespace System.Windows.Controls.DataVisualization.Charting
{
    ...
    public class DateTimeAxis : RangeAxis
    {
        ...
        {
            var dateTimeAxisLabel = label as DateTimeAxisLabel;
        ...
        /// <returns>
        ///     The actual interval to use to determine which values are
        ///     displayed in the axis.
        /// </returns>
        private double CalculateActualInterval(Size availableSize)
        {
            ...
            double interval = CalculateDateTimeInterval(actualDateTimeRange.Minimum, actualDateTimeRange.Maximum,
                out intervalType, availableSize);
        ...
        protected virtual IEnumerable<DateTime> GetMajorAxisValues(Size availableSize)
        {
            ...
            {
                start = IncrementDateTime(start, ActualInterval);
            }

            IEnumerable<DateTime> intermediateDates =
                EnumerableFunctions
                    .Iterate(start, next => IncrementDateTime(next, ActualInterval))
                    .TakeWhile(dateTimeRange.Contains);
        ...
        private static DateTime AlignIntervalStart(DateTime start, double intervalSize, DateTimeIntervalType type)
        {
            ...
            if (intervalSize > 0.0 && intervalSize != 1.0)
            {
                if (type == DateTimeIntervalType.Months && intervalSize <= 12.0 && intervalSize > 1)
                {
                    ...
                    var sizeAdjustedDate = new DateTime(newStartDate.Year, 1, 1, 0, 0, 0);
            ...
            switch (type)
            {
                case DateTimeIntervalType.Years:
                    var year = (int)((int)(newStartDate.Year / intervalSize) * intervalSize);
                    if (year <= 0)
                    {
                        year = 1;
                    }
                ...
                case DateTimeIntervalType.Months:
                    var month = (int)((int)(newStartDate.Month / intervalSize) * intervalSize);
                    if (month <= 0)
                    {
                        month = 1;
                    }
                ...
                case DateTimeIntervalType.Days:
                    var day = (int)((int)(newStartDate.Day / intervalSize) * intervalSize);
                    if (day <= 0)
                    {
                        day = 1;
                    }
                ...
                case DateTimeIntervalType.Hours:
                    var hour = (int)((int)(newStartDate.Hour / intervalSize) * intervalSize);
                ...
                case DateTimeIntervalType.Minutes:
                    var minute = (int)((int)(newStartDate.Minute / intervalSize) * intervalSize);
                    newStartDate = new DateTime(
                        newStartDate.Year,
                        newStartDate.Month,
                        newStartDate.Day,
                ...
                case DateTimeIntervalType.Seconds:
                    var second = (int)((int)(newStartDate.Second / intervalSize) * intervalSize);
                    newStartDate = new DateTime(
                        newStartDate.Year,
                        newStartDate.Month,
                        newStartDate.Day,
                ...
                case DateTimeIntervalType.Milliseconds:
                    var milliseconds = (int)((int)(newStartDate.Millisecond / intervalSize) * intervalSize);
                    newStartDate = new DateTime(
                        newStartDate.Year,
                        newStartDate.Month,
                        newStartDate.Day,
        ...
        [SuppressMessage("Microsoft.Maintainability", "CA1502:AvoidExcessiveComplexity",
            Justification =
                "The method should inspect all variations of time span (millisec to year) and contains long case. Otherwise is simple and readable."
            )]
            ...
            Size availableSize)
        {
            ...
            timeSpan = new TimeSpan((long)(timeSpan.Ticks / rangeMultiplicator));
            ...
            if (range.HasData && ActualLength > 1.0)
            {
                IList<ValueMarginCoordinateAndOverlap> valueMargins =
                    RegisteredListeners.OfType<IValueMarginProvider>()
                        ...
                        .Select(valueMargin => new ValueMarginCoordinateAndOverlap { ValueMargin = valueMargin, })
                        .ToList();
