...
namespace System.Windows.Controls.DataVisualization.Charting
{
    ...
    public class CategoryAxis : DisplayAxis, ICategoryAxis
    {
        ...
        /// </summary>
        private readonly ObjectPool<Line> _majorTickMarkPool;

        /// <summary>
        ///     A pool of labels.
        /// </summary>
        private readonly ObjectPool<Control> _labelPool;
        ...
        public CategoryAxis()
        {
            ...
            GridLineCoordinatesToDisplay = new List<UnitValue>();
        }
        ...
        public Range<UnitValue> GetPlotAreaCoordinateRange(object category)
        {
            ...
            if (Orientation == AxisOrientation.X || Orientation == AxisOrientation.Y)
            {
                ...
                {
                    return new Range<UnitValue>(new UnitValue(maximumLength - upper, Unit.Pixels),
                        new UnitValue(maximumLength - lower, Unit.Pixels));
                }
            }
            else
            {
                double startingAngle = 270.0;
                double angleOffset = 360/Categories.Count;
                double halfAngleOffset = angleOffset/2.0;
                int categoryIndex = Categories.IndexOf(category);
                double angle = startingAngle + (categoryIndex*angleOffset);

                return new Range<UnitValue>(new UnitValue(angle - halfAngleOffset, Unit.Degrees),
                    new UnitValue(angle + halfAngleOffset, Unit.Degrees));
            }
        ...
        {
            if (ActualLength == 0.0 || Categories.Count == 0)
            {
                return null;
            }
            if (position.Unit == Unit.Pixels)
            {
                ...
                var index = (int) Math.Floor(coordinate/(ActualLength/Categories.Count));
                if (index >= 0 && index < Categories.Count)
                {
                    ...
                    return Categories[(Categories.Count - 1) - index];
                }
            }
            else
            {
        ...
        private void RenderOriented(Size availableSize)
        {
            ...
            try
            {
                ...
                GridLineCoordinatesToDisplay.Clear();
                ...
                if (Categories.Count > 0)
                {
                    ...
                    Action<double> placeTickMarkAt =
                        pos =>
                        {
                            ...
                            GridLineCoordinatesToDisplay.Add(new UnitValue(pos, Unit.Pixels));
        ...
        /// </summary>
        /// <param name="value">
        ///     A value which may or may not be able to be
        ///     plotted.
        /// </param>
        /// <returns>
        ///     A value indicating whether a value can be plotted on the
        ///     axis.
        /// </returns>
