...
namespace System.Windows.Controls.DataVisualization.Charting.Primitives
{
    ...
    public class EdgePanel : Panel
    {
        ...
        [SuppressMessage("Microsoft.Design", "CA1011:ConsiderPassingBaseTypesAsParameters",
            Justification = "EdgePanel only has UIElement children")]
        ...
        private static void OnEdgePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            var panel = VisualTreeHelper.GetParent(element) as EdgePanel;
            if (panel != null)
            {
        ...
        public EdgePanel()
        {
            SizeChanged += EdgePanelSizeChanged;
        }
        ...
        [SuppressMessage("Microsoft.Maintainability", "CA1505:AvoidUnmaintainableCode",
            Justification = "Code is by nature difficult to refactor into several methods.")]
        [SuppressMessage("Microsoft.Naming", "CA1725:ParameterNamesShouldMatchBaseDeclaration", MessageId = "0#",
            Justification = "Compat with WPF.")]
        [SuppressMessage("Microsoft.Maintainability", "CA1502:AvoidExcessiveComplexity",
            Justification = "Splitting up method will make it more difficult to understand.")]
        protected override Size MeasureOverride(Size constraint)
        {
            constraint = new Size(ActualWidth, ActualHeight);

            IList<UIElement> leftElements =
                Children.OfType<UIElement>().Where(element => GetEdge(element) == Edge.Left).ToList();
            IList<UIElement> rightElements =
                Children.OfType<UIElement>().Where(element => GetEdge(element) == Edge.Right).ToList();
            IList<UIElement> bottomElements =
                Children.OfType<UIElement>().Where(element => GetEdge(element) == Edge.Bottom).ToList();
            IList<UIElement> topElements =
                Children.OfType<UIElement>().Where(element => GetEdge(element) == Edge.Top).ToList();
            ...
            while (true)
            {
                ...
                {
                    var rightSize = new Size(constraint.Width, _rightRect.Height);
                ...
                {
                    var topSize = new Size(_topRect.Width, constraint.Height);
                ...
                {
                    var leftSize = new Size(constraint.Width, _leftRect.Height);
                ...
                {
                    var bottomSize = new Size(_bottomRect.Width, constraint.Height);
                ...
                {
                    _leftRect = SafeCreateRect(0, maxTopRequestedHeight, maxLeftRequestedWidth,
                        (constraint.Height - maxTopRequestedHeight) - maxBottomRequestedHeight);
                    _rightRect = SafeCreateRect(constraint.Width - maxRightRequestedWidth, maxTopRequestedHeight,
                        maxRightRequestedWidth, (constraint.Height - maxTopRequestedHeight) - maxBottomRequestedHeight);
                    _bottomRect = SafeCreateRect(maxLeftRequestedWidth, constraint.Height - maxBottomRequestedHeight,
                        (constraint.Width - maxLeftRequestedWidth) - maxRightRequestedWidth, maxBottomRequestedHeight);
                    _topRect = SafeCreateRect(maxLeftRequestedWidth, 0,
                        (constraint.Width - maxLeftRequestedWidth) - maxRightRequestedWidth, maxTopRequestedHeight);
        ...
        {
            if (arrangeSize.Width == 0 || arrangeSize.Height == 0 || !ValueHelper.CanGraph(arrangeSize.Width) ||
                !ValueHelper.CanGraph(arrangeSize.Height))
            {
                return arrangeSize;
            }
            ...
            if (!_bottomRect.IsEmpty)
            {
                ...
                foreach (UIElement bottomUIElement in bottomElements)
                {
                    bottomUIElement.Arrange(SafeCreateRect(_leftRect.RightOrDefault(0), workingHeight,
                        (arrangeSize.Width - _leftRect.WidthOrDefault(0)) - _rightRect.WidthOrDefault(0),
                        bottomUIElement.DesiredSize.Height));
            ...
            if (!_topRect.IsEmpty)
            {
                ...
                foreach (UIElement topUIElement in topElements)
                {
                    ...
                    topUIElement.Arrange(SafeCreateRect(_leftRect.RightOrDefault(0), workingTop,
                        (arrangeSize.Width - _leftRect.WidthOrDefault(0)) - _rightRect.WidthOrDefault(0),
                        topUIElement.DesiredSize.Height));
                }
            }
            ...
            if (!_rightRect.IsEmpty)
            {
                ...
                foreach (UIElement rightUIElement in rightElements)
                {
                    rightUIElement.Arrange(SafeCreateRect(workingRight, _topRect.BottomOrDefault(0),
                        rightUIElement.DesiredSize.Width,
                        (arrangeSize.Height - _bottomRect.HeightOrDefault(0)) - _topRect.HeightOrDefault(0)));
            ...
            if (!_leftRect.IsEmpty)
            {
                ...
                foreach (UIElement leftUIElement in leftElements)
                {
                    ...
                    Rect leftRect = SafeCreateRect(workingLeft, _topRect.BottomOrDefault(0),
                        leftUIElement.DesiredSize.Width,
                        (arrangeSize.Height - _bottomRect.HeightOrDefault(0)) - _topRect.HeightOrDefault(0));
