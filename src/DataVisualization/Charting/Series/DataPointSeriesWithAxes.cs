...
namespace System.Windows.Controls.DataVisualization.Charting
{
    ...
    public abstract class DataPointSeriesWithAxes : DataPointSeries, IDataProvider, IRangeProvider, IAxisListener,
        IValueMarginProvider
    {
        ...
        protected static IRangeAxis CreateRangeAxisFromData(object value)
        {
            ...
            if (ValueHelper.TryConvert(value, out dateTime))
            {
        ...
        protected virtual object GetActualDataPointAxisValue(DataPoint dataPoint, IAxis axis)
        {
            ...
            if (axis == InternalActualDependentAxis)
            {
        ...
        /// <param name="oldValue">The old value.</param>
        /// <param name="newValue">The new value.</param>
        protected override void OnDataPointDependentValueChanged(DataPoint dataPoint, IComparable oldValue,
            IComparable newValue)
        {
            if ((null != InternalActualDependentAxis))
            {
                dataPoint.BeginAnimation(DataPoint.ActualDependentValueProperty, "ActualDependentValue", newValue,
                    TransitionDuration, TransitionEasingFunction);
            }
            else
            {
        ...
        /// <param name="oldValue">The old value.</param>
        /// <param name="newValue">The new value.</param>
        protected override void OnDataPointActualIndependentValueChanged(DataPoint dataPoint, object oldValue,
            object newValue)
        {
        ...
        protected override void OnDataPointIndependentValueChanged(DataPoint dataPoint, object oldValue, object newValue)
        {
            ...
            {
                dataPoint.BeginAnimation(DataPoint.ActualIndependentValueProperty, "ActualIndependentValue", newValue,
                    TransitionDuration, TransitionEasingFunction);
            }
            else
            {
        ...
        private void UpdateActualIndependentAxis()
        {
            ...
            {
                var categoryAxis = InternalActualIndependentAxis as ICategoryAxis;
                if (categoryAxis != null)
                {
                    IDataProvider categoryInformationProvider = this;
                    categoryAxis.DataChanged(categoryInformationProvider,
                        categoryInformationProvider.GetData(categoryAxis));
                }
                var rangeAxis = InternalActualIndependentAxis as IRangeConsumer;
                if (rangeAxis != null)
                {
                    IRangeProvider rangeInformationProvider = this;
                    rangeAxis.RangeChanged(rangeInformationProvider, rangeInformationProvider.GetRange(rangeAxis));
        ...
        /// </summary>
        /// <param name="firstDataPoint">The first data point.</param>
        /// <param name="independentAxisPredicate">
        ///     A predicate that returns
        ///     a value indicating whether an axis is an acceptable candidate for
        ///     the series independent axis.
        /// </param>
        /// <param name="independentAxisFactory">
        ///     A function that creates an
        ///     acceptable independent axis.
        /// </param>
        /// <param name="dependentAxisPredicate">
        ///     A predicate that returns
        ///     a value indicating whether an axis is an acceptable candidate for
        ///     the series dependent axis.
        /// </param>
        /// <param name="dependentAxisFactory">
        ///     A function that creates an
        ///     acceptable dependent axis.
        /// </param>
        protected virtual void GetAxes(DataPoint firstDataPoint, Func<IAxis, bool> independentAxisPredicate,
            Func<IAxis> independentAxisFactory, Func<IAxis, bool> dependentAxisPredicate,
            Func<IAxis> dependentAxisFactory)
        {
            Func<IAxis, bool> actualIndependentAxisPredicate =
                axis => independentAxisPredicate(axis) && axis.CanPlot(firstDataPoint.IndependentValue);
            IAxis workingIndependentAxis = null;
            if (InternalActualIndependentAxis == null)
            {
                if (InternalIndependentAxis != null)
                {
                    if (actualIndependentAxisPredicate(InternalIndependentAxis))
                    {
                        workingIndependentAxis = InternalIndependentAxis;
                    }
                    else
                    {
                        throw new InvalidOperationException(
                            Properties.Resources.DataPointSeriesWithAxes_GetAxes_AssignedIndependentAxisCannotBeUsed);
                    }
                }

                if (workingIndependentAxis == null)
                {
                    workingIndependentAxis = SeriesHost.Axes.FirstOrDefault(actualIndependentAxisPredicate);
                }
                ...
                InternalActualIndependentAxis = workingIndependentAxis;

                if (!workingIndependentAxis.RegisteredListeners.Contains(this))
                {
                    workingIndependentAxis.RegisteredListeners.Add(this);
                }
                if (!SeriesHost.Axes.Contains(workingIndependentAxis))
                {
                    SeriesHost.Axes.Add(workingIndependentAxis);
                }
            }

            Func<IAxis, bool> actualDependentAxisPredicate =
                axis => dependentAxisPredicate(axis) && axis.CanPlot(firstDataPoint.DependentValue);
            IAxis workingDependentAxis = null;
            if (InternalActualDependentAxis == null)
            {
                if (InternalDependentAxis != null)
                {
                    if (actualDependentAxisPredicate(InternalDependentAxis))
                    {
                        workingDependentAxis = InternalDependentAxis;
                    }
                    else
                    {
                        throw new InvalidOperationException(
                            Properties.Resources.DataPointSeriesWithAxes_GetAxes_AssignedDependentAxisCannotBeUsed);
                    }
                }

                if (workingDependentAxis == null)
                {
                    workingDependentAxis =
                        InternalActualIndependentAxis.DependentAxes.Concat(SeriesHost.Axes)
                            .FirstOrDefault(actualDependentAxisPredicate);
                }
                ...
                InternalActualDependentAxis = workingDependentAxis;
                ...
                // independent axis.
                if (!SeriesHost.Axes.Contains(workingDependentAxis) &&
                    !InternalActualIndependentAxis.DependentAxes.Contains(workingDependentAxis))
                {
                    SeriesHost.Axes.Add(workingDependentAxis);
                }
            }
        }
