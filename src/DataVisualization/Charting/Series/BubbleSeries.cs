...
namespace System.Windows.Controls.DataVisualization.Charting
{
    ...
    public class BubbleSeries : DataPointSingleSeriesWithAxes
    {
        ...
        {
            get { return (null != SizeValueBinding) ? SizeValueBinding.Path.Path : null; }
            set
            {
                if (null == value)
                {
        ...
        /// </summary>
        private Range<double> _rangeOfActualSizeValues;
        ...
        protected override void PrepareDataPoint(DataPoint dataPoint, object dataContext)
        {
            ...
            var bubbleDataPoint = (BubbleDataPoint) dataPoint;
            bubbleDataPoint.SetBinding(BubbleDataPoint.SizeProperty,
                SizeValueBinding ?? DependentValueBinding ?? IndependentValueBinding);
        }
        ...
        {
            Range<double> newRangeOfActualSizeValues =
                ActiveDataPoints.OfType<BubbleDataPoint>().Select(d => Math.Abs(d.ActualSize)).GetRange();
        ...
        {
            var dataPoint = (BubbleDataPoint) sender;

            dataPoint.BeginAnimation(BubbleDataPoint.ActualSizeProperty, "ActualSize", e.NewValue, TransitionDuration,
        ...
        {
            double maximumDiameter = Math.Min(PlotAreaSize.Width, PlotAreaSize.Height)*
                                     MaximumBubbleSizeAsRatioOfSmallestDimension;

            var bubbleDataPoint = (BubbleDataPoint) dataPoint;

            double ratioOfLargestBubble =
                (_rangeOfActualSizeValues.HasData && _rangeOfActualSizeValues.Maximum != 0.0 &&
                 bubbleDataPoint.ActualSize >= 0.0)
                    ? Math.Abs(bubbleDataPoint.ActualSize)/_rangeOfActualSizeValues.Maximum
                    : 0.0;
