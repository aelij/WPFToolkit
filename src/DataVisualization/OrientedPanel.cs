...
namespace System.Windows.Controls.DataVisualization
{
    ...
    internal class OrientedPanel : Panel
    {
        ...
        private static void OnMinimumDistanceBetweenChildrenPropertyChanged(DependencyObject d,
            DependencyPropertyChangedEventArgs e)
        {
            var source = (OrientedPanel) d;
        ...
        [SuppressMessage("Microsoft.Design", "CA1011:ConsiderPassingBaseTypesAsParameters",
            Justification = "This is an attached property and is only intended to be set on UIElement's")]
        ...
        public static void OnCenterCoordinatePropertyChanged(DependencyObject dependencyObject,
            DependencyPropertyChangedEventArgs eventArgs)
        {
            ...
            var parent = VisualTreeHelper.GetParent(source) as OrientedPanel;
            if (parent != null)
            {
        ...
        /// <param name="eventArgs">Event arguments.</param>
        public static void OnPriorityPropertyChanged(DependencyObject dependencyObject,
            DependencyPropertyChangedEventArgs eventArgs)
        {
            var source = dependencyObject as UIElement;
            if (source == null)
        ...
        {
            SetBinding(ActualLengthProperty,
                new Binding((Orientation == Orientation.Horizontal) ? "ActualWidth" : "ActualHeight") {Source = this});
        }
        ...
        /// <param name="lengthSelector">
        ///     A function that returns a length given
        ///     a UIElement.
        /// </param>
        /// <returns>A sequence of ranges.</returns>
        private static IEnumerable<Range<double>> GetRanges(IEnumerable<UIElement> children,
            Func<UIElement, double> lengthSelector)
        {
            return
                children
                    .Select(child =>
        ...
        [SuppressMessage("Microsoft.Maintainability", "CA1502:AvoidExcessiveComplexity",
            Justification =
                "Linq use artificially increases cyclomatic complexity.  Linq functions are well-understood.")]
        protected override Size MeasureOverride(Size availableSize)
        {
            double offset = 0.0;
            if (Children.Count > 0)
            {
                var totalSize = new Size(double.PositiveInfinity, double.PositiveInfinity);
                ...
                IEnumerable<IGrouping<int, UIElement>> priorityGroups =
                    ...
                    group child by GetPriority(child)
                    into priorityGroup
                        ...
                        select
                            (EnumerableFunctions.Zip(
                                ...
                                (leftElement, rightElement) =>
                                {
                                    ...
                                    return (rightCenterCoordinate - halfRightLength) -
                                           (leftCenterCoordinate + halfLeftLength);
                                }))
                                .Min())
                ...
                IEnumerable<Tuple<int, int>> priorityPairs =
                    EnumerableFunctions.Zip(priorities, priorities.Skip(1),
                        (previous, next) => new Tuple<int, int>(previous, next));

                foreach (Tuple<int, int> priorityPair in priorityPairs)
                {
                    IEnumerable<UIElement> currentPriorityChildren =
                        Children.Cast<UIElement>().Where(child => GetPriority(child) == priorityPair.Item1).ToList();

                    IEnumerable<Range<double>> currentPriorityRanges =
                        GetRanges(currentPriorityChildren, lengthSelector);

                    IEnumerable<UIElement> nextPriorityChildren =
                        Children.Cast<UIElement>().Where(child => GetPriority(child) == priorityPair.Item2).ToList();
