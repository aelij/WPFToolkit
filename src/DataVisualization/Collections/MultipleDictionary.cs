...
namespace System.Windows.Controls.DataVisualization.Collections
{
    ...
    internal class MultipleDictionary<TKey, TValue>
    {
        ...
        [SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", MessageId = "allowDuplicateValues",
            Justification = "Unused parameter exists for API compatibility.")]
            ...
            IEqualityComparer<TValue> valueEqualityComparer)
        {
            ...
            BinaryTree = new LeftLeaningRedBlackTree<TKey, TValue>(
                ...
                (left, right) =>
                    valueEqualityComparer.GetHashCode(left).CompareTo(valueEqualityComparer.GetHashCode(right)));
        }
        ...
        {
            get { return BinaryTree.GetValuesForKey(key).ToList(); }
        }
